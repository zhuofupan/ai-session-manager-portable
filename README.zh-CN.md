...
截图使用临时 demo 数据和 demo 路径，不包含真实本地聊天记录。英文界面截图见 [README.md](README.md)。
...
- 会话列表按项目目录分组，可折叠展开，便于在多个工作区之间切换。
- 会话详情支持文本选择复制、回复折叠、字号调整和右侧导览预览。
...
- 可选 Codex 完成回复后弹窗提醒。
- 内置多套皮肤主题，可在 GUI 里直接切换。
- 首屏优先加载最近会话，后台再补全更多历史，减少启动等待。
...
| `项目目录` | 只显示或派生某个工作目录下的对话；下拉列表较长时会向右展开，减少遮挡 |
| `搜索` | 按标题、ID、目录或账号过滤会话 |
...
| 底部 `Codex` / `cc-switch` 路径 | 点击可重新选择本地 Codex 状态库或 cc-switch 数据库 |
| `帮助` / `更新` | 打开帮助或执行 Git 更新检查 |
